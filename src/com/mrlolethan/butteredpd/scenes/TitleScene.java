...
import com.mrlolethan.butteredpd.effects.Fireball;
...
public class TitleScene extends PixelScene {
	...
	public void create() {
		...
		int gameversion = ButteredPixelDungeon.version();

		if (gameversion != Game.versionCode) {
			//new intro, make older players see it again.
			if (gameversion < 9)
				ButteredPixelDungeon.intro(true);
		...
		float height = title.height +
				(ButteredPixelDungeon.landscape() ? DashboardItem.SIZE : DashboardItem.SIZE * 2);
		...
		DashboardItem btnBadges = new DashboardItem( TXT_BADGES, 3 ) {
			...
			protected void onClick() {
				ButteredPixelDungeon.switchNoFade( BadgesScene.class );
			}
		};
		add( btnBadges );
		...
		DashboardItem btnAbout = new DashboardItem( TXT_ABOUT, 1 ) {
			@Override
			protected void onClick() {
				ButteredPixelDungeon.switchNoFade( AboutScene.class );
			}
		};
		add( btnAbout );
